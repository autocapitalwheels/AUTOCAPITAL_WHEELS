    ...
    <section className="relative w-full overflow-hidden flex items-center bg-[#0a0a0c]"
      ...
      {slides.map((slide, index) => {
        ...
        return (
          ...
          >
            {isVideo ? (
              <video src={slide} className="w-full h-full object-cover object-center" autoPlay muted loop playsInline />
            ) : (
              <img src={slide} alt="Premium Pre-Owned Cars" className="w-full h-full object-cover object-center" />
            )}
          </div>
        );
      })}
      ...
      <div className="container-custom relative z-20 w-full py-16 lg:py-20">
        <div className="max-w-2xl">
          ...
          <div className="flex items-center gap-2 mb-5 animate-fade-in">
            ...
            <span className="text-[10px] font-bold tracking-[0.2em] uppercase text-[#b48d36]">Delhi's Premium Used Cars</span>
          </div>

          {/* Headline */}
          <h1 className="font-display text-4xl sm:text-5xl lg:text-[4rem] xl:text-[4.5rem] font-black tracking-tight text-white leading-[1.08] mb-5 animate-slide-up">
            Trusted Cars.
            <br />
            <span style={{ color: '#b48d36' }}>Trusted Deals.</span>
          </h1>

          <p className="text-neutral-400 text-sm sm:text-base max-w-md font-light leading-relaxed mb-8 animate-slide-up animation-delay-100">
          ...
          {/* Trust Badges */}
          <div className="grid grid-cols-3 gap-4 border-t border-white/8 pt-6 max-w-sm mb-8 animate-slide-up animation-delay-200">
      ...
      {slides.length > 1 && (
        <>
          ...
          <button
            onClick={goPrev}
            className="absolute left-4 top-1/2 -translate-y-1/2 z-30 p-2 rounded-full bg-black/40 border border-white/10 text-white/60 hover:text-white hover:bg-black/70 hover:border-white/20 transition-all duration-200 hidden sm:flex items-center justify-center"
          ...
          <button
            onClick={goNext}
            className="absolute right-4 top-1/2 -translate-y-1/2 z-30 p-2 rounded-full bg-black/40 border border-white/10 text-white/60 hover:text-white hover:bg-black/70 hover:border-white/20 transition-all duration-200 hidden sm:flex items-center justify-center"
